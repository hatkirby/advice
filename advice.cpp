...
verbly::word advice::generateImageNoun() const
{
  ...
  return pictureQuery.first();
}

Magick::Image advice::getImageForNoun(verbly::word pictured) const
{
...
Magick::Image advice::layoutImage(Magick::Image pic, std::string title) const
{
  // Want a 16:9 aspect
...
void advice::sendTweet(Magick::Image pic, std::string title) const
{
...
void advice::run() const
{
  for (;;)
  {
    try
    {
      // Pick a noun to use for the picture.
      verbly::word pictured = generateImageNoun();

      // Find an image of the picked noun.
      Magick::Image pic = getImageForNoun(pictured);

      // Generate the image text.
      std::string title = generator_->generate();

      // Layout the image.
      Magick::Image output = layoutImage(std::move(pic), title);

      // Tweet the image.
      sendTweet(std::move(output), title);
